...
private let CountryCodePath = "CountryCode.sqlite3"
class CountryCode: NSObject {
    
    private var db:COpaquePointer = nil
    class var shared:CountryCode{
        struct DAO{
            static var pred:dispatch_once_t = 0
            static var dao:CountryCode? = nil
        }
    ...
    private func importSqlite(){
        if let dbPath = self.defaultDBFilePath() {
            if let cDBPath = dbPath.cStringUsingEncoding(NSUTF8StringEncoding) {
                if sqlite3_open(cDBPath, &db) == SQLITE_OK {
                    if let filePath = NSBundle.mainBundle().pathForResource("ga_area", ofType: "sql") {
                        do {
                            let sql = try String.init(contentsOfFile: filePath, encoding: NSUTF8StringEncoding)
    ...
    private func defaultDBFilePath() ->String?{
        if let docPath = NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory.DocumentDirectory, NSSearchPathDomainMask.UserDomainMask, true).last {
            let defaultPath = NSString.init(string: docPath).stringByAppendingPathComponent(CountryCodePath)
            let isExist = NSFileManager.defaultManager().fileExistsAtPath(defaultPath)
            if isExist == false {
