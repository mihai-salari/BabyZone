...
class DXPickerHelper: NSObject {
    ...
    class func hm_fetchImageWithAsset(asset: PHAsset?, targetSize: CGSize, needHighQuality: Bool,imageResultHandler: (image: UIImage?, info:[NSObject : AnyObject]?)->Void) -> PHImageRequestID? {
        guard asset != nil else {
            return nil
        }
        let options = PHImageRequestOptions()
        ...
        return PHImageManager.defaultManager().requestImageForAsset(asset!,
                                                                    ...
                                                                    options: options) {
                                                                        (result, info) -> Void in
                                                                        imageResultHandler(image: result, info: info)
        }
    }
