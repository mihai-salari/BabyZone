...
@implementation HMAddressBookHandle

+ (void)getAddressBookDataSource:(HMPersonModelBlock)personModel authorizationFailure:(AuthorizationFailure)failure {
    if(IOS9_LATER) {
...
+ (void)getDataSourceFrom_IOS9_Ago:(HMPersonModelBlock)personModel authorizationFailure:(AuthorizationFailure)failure {
    // 1.获取授权状态
    ...
    for(id personInfo in (__bridge NSArray *)allPeopleArray) {
        HMPersonModel *model = [HMPersonModel new];
        // 5.1获取到联系人
        ...
        model.name = name.length > 0 ? name : nil ;
        // 5.3获取头像数据
        ...
        for (CFIndex j = 0; j < ABMultiValueGetCount(address); j++) {
            ...
            // 详细地址建模
            HMAddressModel *addressModel = [[HMAddressModel alloc] init];
...
+ (void)getDataSourceFrom_IOS9_Later:(HMPersonModelBlock)personModel authorizationFailure:(AuthorizationFailure)failure {
#ifdef __IPHONE_9_0
    // 1.获取授权状态
...
+ (void)addPersonToAddressBook_IOS9_Later:(HMPersonModel *)person failure:(AuthorizationFailure)failure {
    CNAuthorizationStatus status = [CNContactStore authorizationStatusForEntityType:CNEntityTypeContacts];
    if (status != CNAuthorizationStatusAuthorized) {
        failure ? failure() : nil;
        return;
...
+ (void)addPersonToAddressBook_IOS9_Ago:(HMPersonModel *)person failure:(AuthorizationFailure)failure {
    ABAuthorizationStatus status = ABAddressBookGetAuthorizationStatus();
    if (status != kABAuthorizationStatusAuthorized) { // 已经授权
        failure ? failure() : nil;
        return;
