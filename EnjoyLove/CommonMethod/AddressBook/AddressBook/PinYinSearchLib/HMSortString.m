...
#import "UIAlertController+HMAdd.h"

@implementation HMSortString

+ (NSArray *)sortForStringAry:(NSArray *)ary {
...
+ (NSArray *)getAllValuesFromDict:(NSDictionary *)dict {
    ...
    return [valuesAry copy];
}

+ (NSDictionary *)sortAndGroupForArray:(NSArray *)ary PropertyName:(NSString *)name {
    ...
    if ([objc isKindOfClass:[NSString class]]) {
        ...
        for (NSString *str in ary){
            HMSortString *sortString = [HMSortString new];
    ...
    NSString *itemString;
    for (HMSortString *sort in sortAry){
...
+ (NSMutableArray *)sortAsInitialWithArray:(NSArray *)ary {
    ...
    for (NSInteger index = 0; index < ary.count; index++) {
        HMSortString *sortString = ary[index];
        sortString.englishString = [HMSortString transform:sortString.string];
